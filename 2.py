...
class Customer:
    ...
    def calculate_percentages(self, annual_interest_rate, loan_period_years):
        interest_rate_decimal = annual_interest_rate / 100
        current_date = datetime.date.today()
        past_date = current_date - datetime.timedelta(days=365 * loan_period_years)
        if current_date > past_date:
            total_interest_amount = Account.balance * ((1 + interest_rate_decimal) ** loan_period_years - 1)
            Account.balance += total_interest_amount
        else:
            print("No interest earned on the account.")
...
class Human(Customer):
    def __init__(self, name, surname, email, datetime_created):
        self.name = name
        self.surname = surname
        self.email = email
        self.datetime_created = datetime.datetime.now()
